...
export const api = {
  ...
  renameWorkspace: (workspaceId:number, name:string, research_question?:string) => request<Workspace>(`/workspaces/${workspaceId}`, {method:"PATCH", body:JSON.stringify({name, research_question})}),
  deleteWorkspace: (workspaceId:number) => request<{status:string}>(`/workspaces/${workspaceId}`, {method:"DELETE"}),
